...
class MainFunctions(Main):
	...
	def CreateAccount(self,event):
		...
		Button(newFrame,text='Next',font='verdana 9',padx=90,pady=10,bg='green3',fg='black',command=self.CreateAccountSTEP2).place(x=395,y=300)
	...
	def CreateAccountSTEP2(self):
		...
		Label(newFrame,text='email Id',font='verdana 11',bg='white',fg='black').place(x=400,y=280-20)
	...
	def CreateAccountSTEP3(self):
		...
		Button(newFrame,text='Getting Started with New Account',bg='green3',fg='black',padx=20,pady=10,command=self.createaccount__).place(x=395,y=300)
...
class Screens(Main):
    ...
    def ComposeMail(self):
        ...
        title=StringVar()
        Entry(f2,textvariable=title,font='verdana 13',bg='black',fg='white',relief='flat').place(x=80,y=120,width=500)
        Content=Text(f2,bg='black',fg='white')
...
class Settings(Main):
    ...
    def _Settings(self):
        ...
        Button(mainFrame,text='LogOut',bg='white',fg='grey60',command=self.logout).place(x=20,y=80)
    def Profile(self):
        mainFrame=Frame(self.root,bg='white',width=int(self.width),height=int(self.height)-30).place(x=0,y=0)
